...
from models import CraftingPattern
from utils import (
    compute_wci,
    generate_skill_up_fn,
    EMPTY_MAT_COST,
    CATEGORY_NAME_MATERIAL,
)
...
class Simulation:
    def __init__(self, patterns: list, market: dict):
        self.cm_bank = {}
        self.patterns = Simulation.augment_patterns_to_include_cost(
            patterns, market
        )
        self.probability_fn_mapping = Simulation.map_probability_fns(
            self.patterns
        )
    ...
    def map_probability_fns(patterns: dict) -> dict:
        return {
            name: generate_skill_up_fn(p.skill)
    ...
    @staticmethod
    def fmt_freq_dict_based_on_ordered_key_list(
        ordered_keys: list, freq_dict_to_sort: dict
    ) -> dict:
        sorted_dict = {}
    ...
    def pop_materials_from_bank_if_exists(
        self, pattern: CraftingPattern
    ) -> None:
    ...
    def compute_score(
        self, pattern: CraftingPattern, prob_crafting: int, bank_fn
    ):
        pattern_cost = 0
        for name, qty in pattern.materials.items():
    ...
    def step(self, level: int) -> SimulationStep:
        ...
        probabilities = dict(
            filter(lambda x: x[1] != 0.0, probabilities.items())
        )
        ...
        for k, p_crafting in probabilities.items():
            ...
            bank_fn = self.is_in_bank if is_not_m_material else lambda _: False
    ...
    def run_simulation(self, config: RunConfigs) -> None:
        ...
        for i in range(config.simulations):
            ...
            snapshot_end_lv_time = round(
                timeit.default_timer() - snapshot_start_time_sim, 3
            )
        ...
        sim_end_time = round(
            timeit.default_timer() - snapshot_start_time_overall, 3
        )
        ...
        (cost_fifth_p_key, _) = min(
            sim_costs.items(), key=lambda x: abs(cost_fifth_p - x[1])
        )
        path_from_cost_fifth_p_key = Simulation.to_frequency_dict(
            sim_crafting_paths[cost_fifth_p_key]
        )
        print(
            f"\nPath associated with ~5th percentile cost: {path_from_cost_fifth_p_key}"
        )

        (cost_median_p_key, _) = min(
            sim_costs.items(), key=lambda x: abs(cost_median - x[1])
        )
        path_from_cost_median_p_key = Simulation.to_frequency_dict(
            sim_crafting_paths[cost_median_p_key]
        )
        print(
            f"\nPath associated with approx. median cost: {path_from_cost_median_p_key}"
        )

        (cost_ninety_fifth_p_key, _) = min(
            sim_costs.items(), key=lambda x: abs(cost_ninety_fifth_p - x[1])
        )
        path_from_cost_ninety_fifth_p_key = Simulation.to_frequency_dict(
            sim_crafting_paths[cost_ninety_fifth_p_key]
        )
        print(
        ...
        with open(
            f"paths/sim-run-{cost_ninety_fifth_p_key}-{config.simulations}-path.json",
            "w",
            encoding="utf-8",
        ) as file:
            output_cost_path = {}
            output_cost_path["metadata"] = {
                ...
                "sim_path_duration": sim_crafting_paths_output[
                    cost_ninety_fifth_p_key
                ]["sim_path_duration"],
            }

            # Reformatting the output so that this key is in the metadata
            del sim_crafting_paths_output[cost_ninety_fifth_p_key][
                "sim_path_duration"
            ]

            output_cost_path["cost"] = cost_ninety_fifth_p
            output_cost_path["path"] = sim_crafting_paths_output[
                cost_ninety_fifth_p_key
            ]
        ...
        keys_norm = set(
            list(path_from_cost_fifth_p_key.keys())
            + list(path_from_cost_median_p_key.keys())
            + list(path_from_cost_ninety_fifth_p_key.keys())
        )
        keys_norm = self.sort_pattern_keys(keys_norm)

        path_from_cost_fifth_p_key = (
            Simulation.fmt_freq_dict_based_on_ordered_key_list(
                keys_norm, path_from_cost_fifth_p_key
            )
        )
        path_from_cost_median_p_key = (
            Simulation.fmt_freq_dict_based_on_ordered_key_list(
                keys_norm, path_from_cost_median_p_key
            )
        )
        path_from_cost_ninety_fifth_p_key = (
            Simulation.fmt_freq_dict_based_on_ordered_key_list(
                keys_norm, path_from_cost_ninety_fifth_p_key
            )
        )
        ...
        path_from_cost_fifth_p_key_df = pd.DataFrame(
            list(zip(keys_norm, path_from_cost_fifth_p_key.values())),
            columns=["pattern", "num_crafted"],
        )
        sns.set_theme(style="whitegrid")
        sns.barplot(
            x="num_crafted", y="pattern", data=path_from_cost_fifth_p_key_df
        ).set(title="Patterns Crafted: ~5th Percentile")
        plt.savefig("figures/sb_barplot_fifth_p.png")

        plt.clf()

        plt.figure(figsize=(20, 10))
        path_from_cost_median_p_key_df = pd.DataFrame(
            list(zip(keys_norm, path_from_cost_median_p_key.values())),
            columns=["pattern", "num_crafted"],
        )
        sns.set_theme(style="whitegrid")
        sns.barplot(
            x="num_crafted", y="pattern", data=path_from_cost_median_p_key_df
        ).set(title="Patterns Crafted: Approx. Median")
        plt.savefig("figures/sb_barplot_median_p.png")

        plt.clf()

        plt.figure(figsize=(20, 10))
        path_from_cost_ninety_fifth_p_key_df = pd.DataFrame(
            list(zip(keys_norm, path_from_cost_ninety_fifth_p_key.values())),
            columns=["pattern", "num_crafted"],
        )
        sns.set_theme(style="whitegrid")
        sns.barplot(
            x="num_crafted",
            y="pattern",
            data=path_from_cost_ninety_fifth_p_key_df,
        ).set(title="Patterns Crafted: ~95th Percentile")
        ...
        plt.figure(figsize=(20, 10))
        sim_costs_df = pd.DataFrame(
            list(sim_costs.values()), columns=["cost_g"]
        )
        sns.histplot(sim_costs_df.cost_g, kde=True).set(
            title="Crafting Path Cost Distribution"
        )
