...
from models import CraftingPattern, MarketItem, MarketData


NEXUS_HUB_URL_ITEMS_BASE = "https://api.nexushub.co/wow-classic/v1/items"


def nexus_hub_price_url(
    server: str, faction: str, item_unique_name: str
) -> str:
...
async def main(args: dict) -> None:
    ...
    total_unique_materials = list(
        reduce(
            lambda a, b: a | b,
            map(
                lambda prof: set(list(prof.materials.keys())),
                crafting_patterns,
            ),
        )
    )
    ...
    async with aiohttp.ClientSession() as session:
        for unique_item_name in total_unique_materials:
            url = nexus_hub_price_url(
                args.server, args.faction, unique_item_name
            )
            ...
            data = (
                market_item.data[-1]
                if market_item.data
                else MarketData(None, None, None, None)
            )
    ...
    filename_prefix = args.profession_json_file.split(".")[:-1][0].split("/")[
        -1
    ]

    with open(
        f"{filename_prefix}-{args.server}-{args.faction}-market-data.json",
        "w",
        encoding="utf-8",
    ) as file:
